...
uint8_t WireBus::month = 0;
uint8_t WireBus::day = 0;
uint8_t WireBus::hour = 0;
uint8_t WireBus::minute = 0;
uint8_t WireBus::second = 0;
uint8_t WireBus::_b[8] = {0};
uint8_t WireBus::_pageSize = 32;
 ...
 * Write uint8_t
 */
void WireBus::write(unsigned int address, uint8_t data) {
 ...
 */
void WireBus::write(unsigned int address, uint8_t* data, int n) {
	// status quo
 ...
 */
void WireBus::write(unsigned int address, uint8_t* data, int offset, int n) {
	Wire1.beginTransmission(AT24C32_I2C_ADDRESS);
	if (Wire1.endTransmission() == 0) {
		Wire1.beginTransmission(AT24C32_I2C_ADDRESS);
		Wire1.write(address >> 8);
 ...
 * Read uint8_t
 */
uint8_t WireBus::read(unsigned int address) {
	uint8_t b = 0;
	int r = 0;
	Wire1.beginTransmission(AT24C32_I2C_ADDRESS);
	if (Wire1.endTransmission() == 0) {
 ...
 */
void WireBus::read(unsigned int address, uint8_t* data, int n) {
	int c = n;
	int offD = 0;
 ...
 */
void WireBus::read(unsigned int address, uint8_t* data, int offset, int n) {
	Wire1.beginTransmission(AT24C32_I2C_ADDRESS);
	if (Wire1.endTransmission() == 0) {
		Wire1.beginTransmission(AT24C32_I2C_ADDRESS);
		Wire1.write(address >> 8);
		...
		if (Wire1.endTransmission() == 0) {
			...
			while (Wire1.available() > 0 && r < n) {
				data[offset + r] = (uint8_t)Wire1.read();
				r++;
			}
		}
	}
